...
TEST(GMParser, HandleJoinedTags) {

        {
            GMQuery gm_query;
            std::istringstream is("#Header text 1\n");
            std::ostringstream os;
            is >> gm_query;
            os << gm_query;
            ASSERT_EQ(os.str(),"<h1>Header text 1</h1>\n");
        }

        {
            GMQuery gm_query;
            std::istringstream is("##Header text 2\n");
            std::ostringstream os;
            is >> gm_query;
            os << gm_query;
            ASSERT_EQ(os.str(),"<h2>Header text 2</h2>\n");
        }
        {
            GMQuery gm_query;
            std::istringstream is("###Header text 3\n");
            std::ostringstream os;
            is >> gm_query;
            os << gm_query;
            ASSERT_EQ(os.str(),"<h3>Header text 3</h3>\n");
        }
        {
            GMQuery gm_query;
            std::istringstream is("*list element 1\n");
            std::ostringstream os;
            is >> gm_query;
            os << gm_query;
            ASSERT_EQ(os.str(),"<li>list element 1</li>\n");
        }
        {
            GMQuery gm_query;
            std::istringstream is(">some quote 1\n");
            std::ostringstream os;
            is >> gm_query;
            os << gm_query;
            ASSERT_EQ(os.str(),"<blockquote>some quote 1</blockquote>\n");
        }
        {
            GMQuery gm_query;
            std::istringstream is("=>http://some-address.com link\n");
...
TEST(GMParser, HandleJoinedIncludedTags) {
    {
    GMQuery gm_query;
    std::istringstream input("```some text\n## some text\n```\n");
    std::ostringstream os;
